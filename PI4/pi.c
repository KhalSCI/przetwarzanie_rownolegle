/*
 * PI4 - prywatna suma kazdego watku + atomic raz na koniec regionu
 * Dramatycznie mniej operacji atomowych (T razy zamiast num_steps).
 *
 * Plik liczy PI w JEDNYM uruchomieniu: najpierw sekwencyjnie, potem rownolegle,
 * drukuje czasy i przyspieszenie.
 *
 * Kompilacja: gcc -O3 -fopenmp pi.c -o pi4
 */
#include <stdio.h>
#include <time.h>
#include <omp.h>
...
int main(int argc, char* argv[])
{
    ...
    #pragma omp parallel
    {
        ...
        double sum_local = 0.0;    /* prywatny akumulator (rejestr) */
        ...
        #pragma omp atomic
        sum += sum_local;          /* scalanie - raz na watek */
    }
    pi = sum * step;

    ppstop  = clock();
    pewtime = omp_get_wtime();
    printf("%15.12f Wartosc liczby PI rownolegle  <-- PI4: prywatny sum_local + atomic\n", pi);
